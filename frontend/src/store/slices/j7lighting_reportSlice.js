...
const j7lighting_API_URL = `${API_URL}/api/j7lighting`;

// Create axios instance with base URL
const api = axios.create({
  baseURL: j7lighting_API_URL,
  headers: {
...
export const generateJ7LightingReport = createAsyncThunk(
  'J7Lighting/generateReport',
...
const J7LightingSlice = createSlice({
  name: 'J7Lighting',
  initialState: {
    report: null,
    loading: false,
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(generateJ7LightingReport.fulfilled, (state, action) => {
        state.loading = false;
        state.report = action.payload;
        state.error = null;
      })
      .addCase(generateJ7LightingReport.rejected, (state, action) => {
        state.loading = false;
