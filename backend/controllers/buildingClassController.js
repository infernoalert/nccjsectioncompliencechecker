...
const getBuildingClasses = asyncHandler(async (req, res) => {
  const buildingClasses = await BuildingClassification.find({});
  res.status(200).json({
    success: true,
    data: buildingClasses
  });
});
...
const getBuildingClass = asyncHandler(async (req, res) => {
  const buildingClass = await BuildingClassification.findById(req.params.id);
...
const createBuildingClass = asyncHandler(async (req, res) => {
  const buildingClass = await BuildingClassification.create(req.body);
  res.status(201).json({
    success: true,
    data: buildingClass
  });
...
const updateBuildingClass = asyncHandler(async (req, res) => {
  ...
  const updatedBuildingClass = await BuildingClassification.findByIdAndUpdate(
    req.params.id,
    req.body,
...
const deleteBuildingClass = asyncHandler(async (req, res) => {
  ...
  await buildingClass.deleteOne();
  
  res.status(200).json({
    success: true,
    data: { message: 'Building class removed' }
  });
});
